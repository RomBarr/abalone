"""Games or Adversarial Search (Chapter 5) slightly modified to fit abalone game UI"""
...
def query_player(game, state):
    ...
    print("current state:")
    game.display(state)
    print("available moves: (position): (direction1),(direction2),...")
    lst = game.actions(state)
    temp = ()
    for pos in lst:
        if temp != pos[0]:
            print("")
    ...
    if game.actions(state):
        move_string = input('Your move? answer in this format: ((position), (direction)) >> ')
        try:
...
class Game:
    ...
    def display(self, state):
        ...
        for key in ebrd:
            if key in tbl["WHITE"]:
                for w in tbl["WHITE"][key]:
                    ebrd[key].append((w[1], "W"))
            if key in tbl["BLACK"]:
                for b in tbl["BLACK"][key]:
                    ebrd[key].append((b[1], "B"))
