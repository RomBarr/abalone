"""
    This is code from public github repository. I did not create or own or create this code.
"""
...
def depth_first_graph_search(problem, display=False):
    """
     Modified depth first graph search with an added parameter "display" to show the explored nodes and frontier. 
    """
    frontier = [(Node(problem.initial))]  # Stack
...
def breadth_first_graph_search(problem, display=False):
    """
    Modified breadth first graph search with an added parameter "display" to show the explored nodes and frontier. 
    """
    node = Node(problem.initial)
    if problem.goal_test(node.state):
        if display:
